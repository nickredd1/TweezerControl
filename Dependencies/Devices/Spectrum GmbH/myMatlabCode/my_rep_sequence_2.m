...
%**************************************************************************
%
% rep_sequence.m                              (c) Spectrum GmbH, 10/2015
%
%**************************************************************************
%
...
for i=0 : cardInfo.maxChannels-1  
    [success, cardInfo] = spcMSetupAnalogOutputChannel (cardInfo, i, chAmp(i+1), 0, 0, mRegs('SPCM_STOPLVL_ZERO'), 0, 0); % doubleOut = disabled, differential = disabled
    if (success == false)
...
spcMSetupSequenceStep (cardInfo,   0,        0,       0, 1, 0);
...
%Update segment 1 from rectangle to triangle
disp('Press any key to update segment 1');
pause;
tic
error = spcm_dwSetParam_i32 (cardInfo.hDrv, mRegs('SPC_SEQMODE_WRITESEGMENT'), 1);
error = spcm_dwSetParam_i32 (cardInfo.hDrv, mRegs('SPC_SEQMODE_SEGMENTSIZE'), 4096);
...
errorCode = spcm_dwSetData (cardInfo.hDrv, 0, 4096, 1, 0, Signal);
executionTime=toc;
fprintf('Execution time: %0.2f ms\n',executionTime*1e3);

%Implement dynamic sequence jump to next step
disp('Press any key to jump to the next step');
pause; %wait for the user to strike any key before continuing.

%modify sequence steps memory
spcMSetupSequenceStep (cardInfo,   0,        1,       0, 1, 0); 
spcMSetupSequenceStep (cardInfo,   1,        2,       1, 50000, 0);
