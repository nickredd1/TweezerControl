%**************************************************************************
%
% m4i_rep_fifo_single.m                        (c) Spectrum GmbH, 10/2015
%
%**************************************************************************
%
...
for i=0 : cardInfo.maxChannels-1  
    [success, cardInfo] = spcMSetupAnalogOutputChannel (cardInfo, i, cardAmp, 0, 0, mRegs('SPCM_STOPLVL_ZERO'), 0, 0); % doubleOut = disabled, differential = disabled
    if (success == false)
...
[success, cardInfo] = spcMSetupModeRepFIFOSingle (cardInfo, 0, 1, 0, 0); %activate Ch0 w/ chEnableL=1;
if (success == false)
...
%Define waveform
freqResolution=1e6; %Hz
memSamples=samplerate*1/freqResolution; %must be a factor of 1024
scale=2^14/2-1; %8191

t=10*(1:memSamples)/samplerate;

wfm=struct();
wfm.fun='@(j) wfm.amp(j)*sin(2*pi*wfm.freq(j)*t+wfm.phase(j))';
wfm.freq=10e6; %[60:0.5:110]*1e6;
wfm.amp=ones(1,numel(wfm.freq));
wfm.phase=2*pi*rand(1,numel(wfm.freq)); %zeros(1,numel(wfm.freq)); 
...
while endLoop == false
    ...
    if (toc > replayTime_sec) %software defined time
        endLoop = true;
    end
end
