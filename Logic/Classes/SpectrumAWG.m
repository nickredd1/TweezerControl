classdef SpectrumAWG < Device
    ...
    properties
        ...
        % Scaling factor for scaling waveforms to something that can be
        % output successfully by the AWG
        ScalingFactor = 2^13 - 1;
    ...
    methods
        ...
        function output(obj, wfm)
            ...
            % set new waveform with scaling factor!
            errorCode = spcm_dwSetData (obj.CardHandle.hDrv, 0,...
                obj.NumMemSamples, 1, 0, obj.ScalingFactor * wfm.Signal);
            obj.checkError(errorCode);
        ...
        function changeAmplitude(obj, amp)
            ...
            error = 0;
            % 30010 = SPC_AMP0 
            if obj.CardHandle.AO.gainProgrammable == true
                error = error + spcm_dwSetParam_i32 (...
                    obj.CardHandle.hDrv, 30010 + channel * 100, amp);  
            end

            [success, obj.CardHandle] = spcMCheckSetError(error,...
                obj.CardHandle);
        end
    end
